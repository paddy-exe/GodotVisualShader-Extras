extends RefCounted
class_name ShaderFuncRefFuckkkkk
...
#
#func get_unique_funcs(sfunc_names_in:Array)->String:
#	var s:String = ""
#	for n in sfunc_names_in:
#		if not n in sfuncs_already_in_global:
#			if n in shader_funcs:
#				s += shader_funcs[n]
#			else:
#				s += "!!WARNING: %s not in shader funcs" % n
#			sfuncs_already_in_global[n]=n
#	prune()
#	return s
